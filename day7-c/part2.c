...
ull concat_nums(ull a, ull b) {
    size_t b_len = count_digits(b);
    ull offset = 1;
...
int check_if_possible(equation *eq) {
    ...
    for (size_t i = 0; i < eq->nums.len; i++) {
        ...
        for (size_t j = 0; j < length; j++) {
            ...
            vec_insert(&results, j, num * next);
            vec_push(&results, num + next); 
            vec_push(&results, concat_nums(num, next)); 
        }
    }    
